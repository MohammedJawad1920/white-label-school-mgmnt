...
test.describe('E2E-012: Leave Polling', () => {
  test('On-campus list refreshes automatically every 30 seconds', async ({ page }) => {
  ...
  test('Marking student departed updates list immediately', async ({ page }) => {
    await loginAs(page, 'admin');
    ...
    if (await leaveRow.isVisible()) {
      ...
      await expect(page.locator(`[data-leave-id="${leaveId}"][data-status="ACTIVE"]`)).toBeVisible();

      // Navigate to on-campus list
      await page.goto('/admin/leave/on-campus');

      // Verify student no longer appears in on-campus list
      await expect(page.locator(`[data-leave-id="${leaveId}"]`)).not.toBeVisible();
    }
  });

  test('Marking student returned updates list immediately', async ({ page }) => {
    await loginAs(page, 'admin');
    await page.goto('/admin/leave');
    ...
    if (await leaveRow.isVisible()) {
      const leaveId = await leaveRow.getAttribute('data-leave-id');
      const studentName = await leaveRow.locator('[data-field="studentName"]').textContent();
      ...
      await expect(page.locator(`[data-leave-id="${leaveId}"][data-status="COMPLETED"]`)).toBeVisible();
