...
test.describe("E2E-006: Notifications", () => {
  test("Notification bell displays unread count, click navigates, mark all read clears badge", async ({
    page,
  }) => {
    ...
    const badge = page.locator(
      '[data-testid="notification-bell"] [data-testid="unread-badge"]',
    );
    ...
    if (initialCount > 0) {
      ...
      await page.waitForResponse((resp) =>
        resp.url().includes("/notifications/mark-all-read"),
      );
  ...
  test("Mark all read endpoint returns { updated: N } not { updatedCount: N } (regression: CR-FE-005)", async ({
    page,
  }) => {
    await loginAs(page, "admin");
    await page.goto("/admin/dashboard");
  ...
  test("Notification list reads from .data.data not .data.notifications (regression: CR-FE-004)", async ({
    page,
  }) => {
    await loginAs(page, "admin");
