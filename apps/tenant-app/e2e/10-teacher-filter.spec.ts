...
test.describe("E2E-010: Teacher Class Filter", () => {
  test("Teacher dashboard renders without blank screen (regression: CR-FE-022)", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator('h1:has-text("Teacher Dashboard")'),
    ).toBeVisible();
  });

  test("Record Attendance shows only teacher own class students, not all tenant students", async ({
    page,
  }) => {
    await loginAs(page, "teacher");
