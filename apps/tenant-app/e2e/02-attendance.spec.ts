...
import { getAdminToken, createApiClient, seedAcademicSession, seedBatch, seedClass, seedStudent, activateSession } from './helpers/api';

test.describe('E2E-002: Record Attendance', () => {
  test('Admin records attendance for 3 students, data persists on reload', async ({ page }) => {
    // Setup test data via API
    const token = await getAdminToken();
    const client = createApiClient(token);

    const sessionId = await seedAcademicSession(client, 'E2E 2026-27', '2026-06-01', '2027-05-31');
    ...
    const student1 = await seedStudent(client, batchId, classId, 'S001', 'Student One');
    const student2 = await seedStudent(client, batchId, classId, 'S002', 'Student Two');
    const student3 = await seedStudent(client, batchId, classId, 'S003', 'Student Three');
    ...
    await page.waitForSelector('[data-testid="attendance-form"]');
    expect(await page.locator(`[data-student-id="${student1}"] [data-status="Present"]`).isChecked()).toBe(true);
    expect(await page.locator(`[data-student-id="${student2}"] [data-status="Absent"]`).isChecked()).toBe(true);
    expect(await page.locator(`[data-student-id="${student3}"] [data-status="Late"]`).isChecked()).toBe(true);
  });
