...
test.describe('E2E-011: Feature Flags', () => {
  test('SuperAdmin creates tenant with all 10 feature flags visible', async ({ page }) => {
    // This test requires SuperAdmin app context
    // For now, we'll test via API

    // Login as superadmin via API
    const saLoginRes = await axios.post(`${API_BASE_URL}/superadmin/auth/login`, {
    ...
    // Get tenant features
    const featuresRes = await client.get(`/superadmin/tenants/${tenantId}/features`);
  ...
  test('Admin cannot access disabled feature route (403 FEATURE_DISABLED)', async ({ page }) => {
    ...
    await page.waitForSelector('text=FEATURE_DISABLED', { timeout: 5000 }).catch(async () => {
      // Or might redirect with a toast
      await expect(page.locator('text=feature is not enabled')).toBeVisible();
    });
  });

  test('Feature flag changes take effect immediately without cache', async ({ page }) => {
    await loginAs(page, 'admin');
    ...
    await expect(page.locator('text=FEATURE_DISABLED')).toBeVisible().catch(async () => {
      await expect(page.locator('text=feature is not enabled')).toBeVisible();
    });
  });
});
