...
test.describe("E2E-003: Leave Lifecycle", () => {
  test("Guardian submits leave, Admin approves, attendance marked Excused", async ({
    page,
  }) => {
    // Login as guardian
    ...
    await expect(page.locator('[data-testid="leave-status"]')).toHaveText(
      "PENDING",
    );
  ...
  test("Leave state machine transitions correctly", async ({ page }) => {
    ...
    const leaveId = await seedLeaveRequest(
      client,
      "student-id",
      "2026-10-01",
      "2026-10-03",
