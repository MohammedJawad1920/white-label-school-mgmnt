...
test.describe("E2E-005: Fees and Payments", () => {
  test("Admin raises fee charge, records payment, Guardian sees updated balance", async ({
    page,
  }) => {
    // Setup via API
    const token = await getAdminToken();
    const client = createApiClient(token);
    const chargeId = await createFeeCharge(
      client,
      "student-id",
      5000,
      "2026-09-30",
    ...
    await expect(
      page.locator(`[data-charge-id="${chargeId}"] [data-field="amount"]`),
    ).toHaveText("5000");
    await expect(
      page.locator(`[data-charge-id="${chargeId}"] [data-field="balance"]`),
    ).toHaveText("5000");

    // Record payment
    await page.click(
      `[data-charge-id="${chargeId}"] button:has-text("Record Payment")`,
    );
  ...
  test("Payment request includes amountPaid and paidAt fields (regression: CR-FE-006)", async ({
    page,
  }) => {
    await loginAs(page, "admin");
    await page.goto("/admin/fees");
  ...
  test("Overpayment guard prevents recording more than outstanding balance", async ({
    page,
  }) => {
    await loginAs(page, "admin");
    await page.goto("/admin/fees");
