...
test.describe('E2E-004: Exam Lifecycle', () => {
  test('Full exam lifecycle: create, enter marks, publish, verify grades, download PDF', async ({ page }) => {
    ...
    await page.fill('[data-student="student-1"][data-subject="math"] input', '85');
    await page.fill('[data-student="student-1"][data-subject="english"] input', '40');

    // Student 2: Math=AB (absent), English=75 (C+) → Overall AB
    await page.check('[data-student="student-2"][data-subject="math"] input[type="checkbox"][value="absent"]');
    await page.fill('[data-student="student-2"][data-subject="english"] input', '75');
    ...
    await expect(page.locator('[data-student="student-1"] [data-field="overallStatus"]')).toHaveText('FAIL');
    await expect(page.locator('[data-student="student-1"][data-subject="math"] [data-field="grade"]')).toHaveText('A');
    await expect(page.locator('[data-student="student-1"][data-subject="english"] [data-field="grade"]')).toHaveText('F');

    // Student 2: Has AB (absent)
    await expect(page.locator('[data-student="student-2"][data-subject="math"] [data-field="grade"]')).toHaveText('AB');
