/**
 * StudentTimetablePage — Read-only timetable for the student's class.
 *
 * Fetches all timetable slots (no teacherId filter).
 * The backend only returns slots for the student's enrolled class.
 * Read-only: no create/delete actions.
 *
    ...
    >
      {[1, 2, 3, 4, 5].map((i) => (
        <div key={i} className="flex gap-2">
          <div className="h-12 w-20 bg-muted rounded shrink-0" />
          {DAYS.slice(0, 5).map((d) => (
            <div key={d} className="h-12 flex-1 bg-muted rounded" />
          ))}
        </div>
      ))}
    </div>
...
function SlotCell({ slot }: { slot: TimeSlot }) {
  return (
    <div className="rounded-md border border-l-2 border-l-primary/60 bg-primary/5 px-2 py-1.5 text-xs">
        ...
        (slots.length > 0 || periods.length > 0) && (
          <div className="overflow-x-auto -mx-4 md:mx-0 rounded-lg border">
            <div
              ...
              style={{ minWidth: `${activeDays.length * 120 + 80}px` }}
            >
              {/* Column headers */}
              <div role="row" className="flex bg-muted/50 border-b">
                ...
                {activeDays.map((day) => (
                  <div
                    key={day}
                    role="columnheader"
                    className={`flex-1 min-w-[100px] px-2 py-2.5 text-xs font-semibold text-center border-l${day === todayDay ? " bg-primary/5 text-primary" : ""}`}
                  >
                    {day.slice(0, 3)}
                    {day === todayDay && (
                    ...
                    {activeDays.map((day) => {
                      ...
                      return (
                        <div
                          ...
                          className="flex-1 min-w-[100px] p-1.5 border-l flex flex-col gap-1"
                        >
                          {cellSlots.map((slot) => (
