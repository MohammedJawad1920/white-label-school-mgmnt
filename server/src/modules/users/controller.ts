...
export async function createUser(req: Request, res: Response): Promise<void> {
  ...
  if (!name || !email) {
    send400(res, "name and email are required");
    return;
  }
  // CR-20: password is optional; validate only if provided
  ...
  let isAutoGenerated: boolean;
  if (password === undefined || password === null) {
    // Generate a secure 12-char password: alphanumeric + symbols, no ambiguous chars
    const charset =
      "ABCDEFGHJKLMNPQRSTUVWXYZabcdefghjkmnpqrstuvwxyz23456789!@#$%^&*";
    const bytes = crypto.randomBytes(12);
    finalPassword = Array.from(bytes)
  ...
  const passwordHash = await bcrypt.hash(finalPassword, config.BCRYPT_ROUNDS);
  ...
  try {
    ...
    // CR-20: return temporaryPassword only once if auto-generated; never stored in plain text
    res.status(201).json({
      user: formatUser(result.rows[0]!),
      temporaryPassword: isAutoGenerated ? finalPassword : null,
    });
  } catch (err: unknown) {
    if (
...
export async function deleteUser(req: Request, res: Response): Promise<void> {
  ...
  const existing = await pool.query<{ id: string; roles: UserRole[] }>(
  ...
  // D-05 fix: LASTADMIN guard — prevent deleting the last Admin in the tenant.
  ...
  if (targetUser.roles.includes("Admin")) {
    ...
    if (parseInt(adminCount.rows[0]?.count ?? "0", 10) <= 1) {
      send409(
        res,
        "Cannot delete: this is the last admin of the tenant",
        "LASTADMIN",
      );
      return;
    }
